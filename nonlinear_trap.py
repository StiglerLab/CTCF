...
def correct_linker_soft_trap(force, dist, k1_app, k2_app, beta_dagger1, beta_dagger2, k_dagger1, k_dagger2, width1, width2):
    ...
    for i in range(len(dist)):
        kappa_1 = np.pi * force[i]*k_dagger1/(2*beta_dagger*k_dagger*k1_app*width1)
        kappa_2 = np.pi * force[i]*k_dagger2/(2*beta_dagger*k_dagger*k2_app*width2)
        x_soft1 = 2 * width1/np.pi * np.arcsin(kappa_1)
        x_soft2 = 2 * width2/np.pi * np.arcsin(kappa_2)

        ext_linker = dist[i] - x_soft1 - x_soft2
        f_true = force[i]/(beta_dagger*k_dagger)
